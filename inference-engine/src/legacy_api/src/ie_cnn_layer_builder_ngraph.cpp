...
CNNLayer::Ptr createSubGraphLayer(const std::shared_ptr<ngraph::Node>& layer) {
    ...
    TensorIterator::Body body;
    {
        ...
        for (auto &in : body.inputs) {
            ...
            auto input_to = getInputTo(in);
            if (input_to.size() != 1)
                continue;

            auto consumer_layer = input_to.begin()->second;
            auto consumer_layer_name = consumer_layer->name;

            auto consumer_in_port_set = consumer_layer->insData;
            auto found = std::find_if(consumer_in_port_set.begin(), consumer_in_port_set.end(),
                                      [&in] (const DataWeakPtr &wptr) { return wptr.lock() == in; });
            IE_ASSERT(found != consumer_in_port_set.end());
            auto consumer_port_idx = std::distance(consumer_in_port_set.begin(), found);
    ...
    for (const auto& desc : tensor_iterator->get_input_descriptions()) {
        ...
        if (type_name == "SliceInputDescription") {
            ...
            auto body_output_idx = input_desc->m_body_value_index;
    ...
    if (auto loop_op = dynamic_cast<const ngraph::op::v5::Loop*>(layer.get())) {
        ...
        res->params["loop_trip_count_idx"] = std::to_string(0);
        res->params["loop_execution_condition_idx"] = std::to_string(1);
    }
